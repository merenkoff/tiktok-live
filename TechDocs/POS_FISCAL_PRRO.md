...
Статус: **фазы 1, 2а, 3 и 4 завершены** — весь бэкенд фискализации готов: схема
(миграция 024), шифрование секретов, настройки, контракт `FiscalProvider`,
таксономия ошибок, маппинг, лимитер, смены, реестр документов и оркестрация
чекаута. **Ни один адаптер реального провайдера не написан**, поэтому код
дормантен: `BUILT_IN` в `providers/index.ts` пуст.
...
> provider:'checkbox'}` ещё в фазе 1 — адаптера нет, и это даст 503 на **каждую
> продажу**:
> ```bash
> psql "$DATABASE_URL" -c "select store_id, provider from pos_fiscal_settings where enabled"
> ```
> Должно быть пусто. Экран настроек теперь отдаёт `adapter_available`, чтобы
> фаза 6 предупреждала об этом сама.

Следующий шаг — **фаза 5** (хост-фронт). Она **обязательна** до включения
фискализации где бы то ни было: сегодня касса прячет ошибку продажи за
непрозрачной модалкой, а офлайн-outbox ретраит постоянный отказ каждые 30 с
вечно (`pos/src/offline/sync.ts` считает бэкофф от `createdAt`, а не от
последней попытки). Регистрации не требует. Порядок работ изменён относительно
...
| Источник печатного чека | **Решаем позже** (фаза 8) | Адаптер обязан возвращать И структурные поля, И готовый текст провайдера; выбор — колонка `receipt_source`, инертна до фазы 8 |
| Налоговый код | Store-default + override на товаре | НДС — свойство товара; размеры/цвета его делят |
| Полный офлайн-режим ПРРО | Вне области v1 | Требует диапазонов офлайн-номеров от ДПС и завязан на конкретного провайдера |
...
| **2б** | Адаптер Checkbox — **делаем последним**, нужна песочница для классификации ошибок | ⬜ |
...
- [ ] Завести тестовый аккаунт Checkbox (`my.checkbox.ua`), получить license key,
      PIN кассира и **коды налоговых ставок** (нужны для `default_tax_code`).
- [ ] Снять реальные пары запрос/ответ в `src/__tests__/fixtures/checkbox/*.json`:
      успех, дубликат, закрытая смена, протухшая авторизация, 429, отказ валидации.
