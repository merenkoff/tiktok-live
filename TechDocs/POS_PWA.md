...
новий бандл. Закрито випуском **2.3.0** (2026-09-24): `module-tables-v2.3.0`
і `module-vertical-cafe-v2.3.0`; решта модулів змінилася лише на рядок
`shells`, який `runsInShell` і так покриває, тож лишаються на своїх версіях.
...
- **Сканера-«клавіатури»** (`ScanWedge`). На планшеті він не монтується
  зовсім: USB-сканера там нема, а фокус у його прихований `<input>` — це і є
  екранна клавіатура, що вилазила на `/tablet/register` при відкритті й після
  кожної закритої накладки і читалася як «поле пошуку саме стало активним»
  (2026-09-26). Два шари навмисно: `ScanWedge` читає `usePosShell()` з
  `@pos/platform` і під `'tablet'` повертає `null` — але цей код компілюється
  в кожен remote-бандл і доїжджає до магазину лише з перепублікацією
  вертикалі; тому `RegisterPage` ще й рахує `catalogActive` як `false` під
  `'tablet'` — `active` у каталогах керує тільки сканером, і цей гейт на
  хості вимикає його в уже опублікованих бандлах одразу. Поле пошуку саме по
  собі ніхто не фокусує; клавіатура — лише після тапу по ньому.
