...
Навчання **не перезаписує** запис, який власник очистив: код із `blocked_at`
повертається в `skipped` з `reason: 'blocked'` і не рахується в `accepted`. Так само
воно не перебиває ручну назву — `manual` має найвищий пріоритет. Обидва правила живуть
в `ingestGtinResults`, тож діють і для batch, і для dump-seeding.
