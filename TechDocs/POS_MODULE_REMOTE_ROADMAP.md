...
## #14 — Вертикалі продажів (sales verticals)

**Статус: фази 1–4 зроблено, лишилася фаза 5 (прибирання + реліз 2.0.0).**

Екран продажу перестав бути одним неподільним екраном. `pos_stores.vertical`
задає схему атрибутів товару, одиниці й модуль `vertical-<id>`, який постачає
каталожну половину `/register` через `ModuleDescriptor.sales`. Каркас (кошик,
оплата, ПРРО, чек) лишається в хості; вбудований `vertical-clothing` — fallback
для всіх випадків «модуля немає».

Перша вертикаль поза застосунком — `vertical-flowers`, доставляється тим самим
механізмом, що й `tiktok-live`.

Повний опис, фази й чекліст «як додати вертикаль» — `TechDocs/POS_VERTICALS.md`.
