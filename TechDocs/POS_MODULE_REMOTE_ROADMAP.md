...
Сделано: #1 (seam, не финал), #3, #5, #6, #7, #8, #9, #10.
Механизм операбельный per-store и подписанный. Осталось до раздачи модуль-
ремоутов из отдельного репо: #4 (CSS вне `pos/src`), #2 (CI-публикация в
иммутабельные пути), #1 (полноценный контракт версий), прод-ключ подписи.
