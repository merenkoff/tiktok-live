...
1. **Контрольне число і `mac` каса локально порахувати не може** — формули
   в документації немає, а `mac` = хеш ланцюжка транзакцій, який веде ТП
   Checkbox. Чек, надрукований касою без зв'язку, може нести лише
   `fiscal_code`, `fiscal_date` і відмітку «ОФЛАЙН»; контрольне число і QR
   з'являються після `sell-offline`.
