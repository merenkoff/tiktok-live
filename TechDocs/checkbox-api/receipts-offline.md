...
## Перевірено на демо-касі 2026-09-12

- `go_offline_date` на 8 хвилин у минулому — прийнято.
- `fiscal_date` у минулому — прийнято і збережено як є; `created_at` окремо.
- Відповідь несе `control_number` (4 цифри) і `tax_url` з `mac`, коли клієнт
  не слав жодного з них.
- **`previous_receipt_id` слати не можна.** Значення, яке Checkbox щойно
  повернув для попереднього чека, відхилено: 400
  `receipt.previous_id_last_id_differs`. Поле необовʼязкове — не шлемо.
- **Чек не може бути датований раніше відкриття зміни:** 400
  `date.fiscal_date_logic`.

Деталі та наслідки — `TechDocs/POS_FISCAL_OFFLINE.md`, розділ «Прогон на
демо-кассе».
...
1. ~~**Контрольне число і `mac` каса локально порахувати не може**~~ —
   ...
   **необов'язковий**: реальні чеки перевіряються посиланням без нього
   ...
   в неї є офлайн.
   **Контрольне число — ні, і це виправлення виправлення (2026-09-12, за
   «Описом API Фіскального Сервера», `../dps-prro-api/`).** Воно — хвіст
   фіскального номера офлайн-документа (`<сесія>.<номер>.<контрольне число>`)
   і рахується як CRC32 від рядка, що починається з **«секретного числа»**
   (`OfflineSeed`, яке Сервер видає ПРРО разом з номером офлайн-сесії) і
   закінчується **гешем попереднього документа сесії**. Обидва є лише в
   ПРРО, тобто в Checkbox. Тому `control_number` у відповіді `sell-offline`
   формує Checkbox, а необов'язкове однойменне поле запиту — для того, хто
   сам веде сесію (їхня Kasa), не для інтегратора над хмарним API. Каса без
   зв'язку може надрукувати номер, дату, суму, ФН, «ОФЛАЙН» і QR; контрольне
   число з'явиться після `sell-offline`.
