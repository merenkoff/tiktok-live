...
|---|---|---|---|
| **0. Исследование на песочнице** | Документация снята (`checkbox-api/`); **прогон на демо-кассе сделан 2026-09-12**, фикстуры `offline_*.json` в репозитории, результаты и два найденных бага — в разделе «Прогон на демо-кассе» ниже. Скрипт `npm run fiscal:sandbox:offline -- --ask --go [--outage=N] [--sales=N]` живёт аварию в реальном времени (коды → ожидание → чеки с настоящими отметками → ожидание → реконнект с `go-offline` задним числом). Осталось: повторный прогон после правок — доставка цепочки из нескольких чеков не проверена | 1–2 дня | 🟡 прогон сделан, повтор после правок |
...
### Что прогон не покрыл

Чеки 2 и 3 до ДПС так и не дошли (их убил баг 5), поэтому **не проверено**:
порядок доставки цепочки из нескольких чеков, поведение `go-online` с
неотправленными документами и повторный `sell-offline` того же кода. Это
повод прогнать ещё раз после текущих правок — тем же скриптом, он уже не
шлёт указатель.

## Открытый вопрос фазы 3: код, выданный во время аварии (2026-09-12)

Замечен владельцем при чтении ответа `get-offline-codes`: у кода есть
`created_at` — дата выдачи налоговой. Отсюда следует ограничение, которого
нет ни в одном нашем тесте: **сессия не может начинаться раньше, чем выдан
код, которым её открывают** — иначе мы объявляем аварию временем, когда
этого номера ещё не существовало.

В случае C это обычно выполняется само: касса арендует коды, пока онлайн, а
авария начинается позже. Но есть дыра — **сервер во время аварии кассы
остаётся онлайн**, и крон `*/10` продолжает пополнять пул. Если пул опустел
и `openSession` возьмёт для `go-offline` код, выданный уже **после** начала
аварии, получится ровно та несостыковка. Смягчает риск то, что
`takeFreeCodes` берёт коды по возрастанию `serial_id`, то есть самые старые
первыми; гарантией это не является.

Чего не хватает, если ограничение реально: мы **не храним** `created_at`
провайдера — в `pos_fiscal_offline_codes` есть только наш `fetched_at`
(когда строку записали мы) и `serial_id`. Чтобы выбирать код по дате выдачи,
понадобится миграция и правка `refillOfflineCodes`.

**Прогон 2026-09-12 это ограничение не проверил и не опроверг:** коды были
выданы в 04:56, сессия начата в 04:58 — порядок соблюдён, и отказа не
последовало. То есть ограничение либо отсутствует, либо просто не было
нарушено. Специально ломать порядок на демо-кассе ради ответа пока не стали:
риск невелик (`takeFreeCodes` берёт самые старые коды первыми), а цена
проверки — заведомо испорченный чек в кабинете. Вернуться к этому, если
`date.fiscal_date_logic` или подобный код появится на коде, а не на смене.
