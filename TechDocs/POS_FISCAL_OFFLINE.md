...
| **3. Касса без связи (случай C)** | Касса арендует коды, штампует чек и сама собирает QR (`mac` необязателен — проверено реальным чеком); **контрольное число посчитать нельзя** — оно из секретного числа сессии и хеша цепочки, которые есть только у ПРРО (Checkbox), см. `dps-prro-api/`. Форма фазы зависит от одного ответа: допустим ли бумажный офлайн-чек без контрольного числа до синхронизации. Да → облачный путь, Kasa не нужна; нет → локальный компонент Checkbox Kasa на кассе. Формулировка вопроса — в «Открытых вопросах» | 5–6 дней | ⬜ ждёт ответа Checkbox |
...
**Остался ровно один вопрос, и он не про код: допустимо ли отдать покупателю
бумажный офлайн-чек без контрольного числа, если оно появится в ДПС при
синхронизации?** Документ ДПС бумагу не регулирует (это Положення № 13); два
источника — пояснение ДПС и образец офлайн-чека Checkbox — показывают
контрольное число на чеке. Онлайн-чеки (в т.ч. реальный чек магазина) тут
ничего не доказывают: у онлайн-документов контрольного числа нет вообще,
номер назначает Сервер. Спрашивать поддержку Checkbox надо именно так:
*«ми друкуємо офлайн-чек до `sell-offline`, контрольне число з'являється лише
після; чи допустимий такий паперовий чек?»* — вопрос «как работает офлайн»
они уже ответили, и это был ответ не на то.

Две развилки в зависимости от ответа:
- **допустимо** → случай C делается на облачном пути: аренда кодов кассе,
  штамп и печать без контрольного числа, `sell-offline` при синхронизации,
  контрольное число — на допечатку/е-чек. Kasa не нужна;
- **недопустимо** → единственный способ иметь контрольное число на бумаге в
  момент продажи — тот, кто ведёт сессию, стоит на кассе: локальный
  компонент Checkbox Kasa (или свой ПРРО). Тогда актуальна её спецификация.
