...
| **3. Касса без связи (случай C)** | **Пересматривается 2026-09-11.** Касса арендует коды, штампует чек, сама считает контрольное число (алгоритм ДПС) и сама собирает QR (`mac` необязателен — проверено реальным чеком), отправка — `sell-offline` с нашим же `control_number`. Локальный компонент Checkbox Kasa — запасной путь, если алгоритм или приём `control_number` не сойдутся. Блокеры в «Открытых вопросах» | 5–6 дней | ⬜ ждёт двух проверок |
...
- ESC/POS-макет (`pos/src-tauri/src/hardware/receipt.rs`) печатает «ОФЛАЙН»
  ...
  синхронізації з ПРРО». **Эта строка — временная:** как только мы начнём
  собирать `tax_url` сами (ФН ПРРО + код + дата/время/сумма), офлайн-чек
  получит настоящий QR сразу, и ветка останется только для документов, у
  которых ФН регистратора ещё не известен;
...
- **QR мы собираем сами.** ~~Считалось, что без `mac` ссылка неполная.~~
  **Ошибка, исправлена в тот же день по реальному чеку магазина:**
  ...
  (константа регистратора, приходит в `registerState()`; **хранить её мы пока
  негде — это и есть работа**). `mac` у Checkbox — это
...
- **Контрольное число — тоже считаем сами.** Его формирует сам ПРРО
  односторонней хеш-функцией над датой, временем и суммой «всього»; функция
  одна для всех ПРРО в данный период, публикуется ДПС в Электронном кабинете,
  механизм — в «Описі API фіскального сервера». А в
  `OfflineReceiptSellPayload` у Checkbox `control_number` — **необязательное
  поле запроса**: посчитав его у себя, мы передаём то же значение при
  отправке, и бумага совпадает с записью в ДПС.

**Решение от 2026-09-11 (случай C через Checkbox Kasa) пересматривается.**
Оно опиралось ровно на тот вывод, который оказался неверным: «касса не может
сформировать реквизиты офлайн-чека». Может. Тогда случай C делается на
облачном пути, без локального компонента на каждой кассе: аренда кодов →
штамп и печать полного чека на кассе → отправка `sell-offline` с нашим же
`control_number`. Провайдер остаётся прокладкой до ДПС — то, к чему мы и
идём.

Что нужно проверить, прежде чем закрывать выбор:
1. **Алгоритм контрольного числа** — «Опис API фіскального сервера» из
   Электронного кабинета ДПС (нужен КЕП владельца; из песочницы разработки
   доступа нет);
2. **принимает ли Checkbox наш `control_number`** в `sell-offline` и не
   перетирает ли его своим — прогон на песочнице (фаза 0);
3. формат `sm`/`time` в ссылке: у реального чека `time=1840` и `sm=143.5`, у
   Checkbox — `16:07:28` и `563.00`, то есть кабинет разбирает мягко; какой
   формат печатать нам, определить на прогоне.

Спецификация local API Checkbox Kasa остаётся полезной как запасной путь
(если п. 1 или 2 не сойдётся) — снять её всё равно стоит.
