...
- **Сортировка пула по `serial_id` — под вопросом.** `takeFreeCodes` и
  `listLeasedCodes` (`src/pos/fiscal/offline/pool.ts`) сортируют
  `ORDER BY serial_id ASC NULLS LAST, id ASC`, а комментарий миграции 027 к
  колонке утверждает «codes are handed out in serial order». Если число
  зависит от ответа, то два пополнения дадут пересекающиеся `serial_id`, и
  порядок выдачи перестанет быть порядком получения. Более честный ключ —
  наш собственный `id` (порядок вставки, то есть порядок ответа провайдера,
  в котором неиспользованные коды идут старыми вперёд). **Не менял** —
  решение за владельцем, поведение сегодня не ломается: внутри одного
  пополнения порядок верный, а коды тратятся быстрее, чем накапливаются
  пополнения.
