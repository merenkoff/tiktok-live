...
|---|---|---|---|
| **0. Исследование на песочнице** | Документация снята (`checkbox-api/`); письмо в поддержку Checkbox по вопросам 1–2 ниже отправлено 2026-09-11; прогон на тестовой кассе — скрипт `npm run fiscal:sandbox:offline -- --ask --go [--minutes=N]` (`CHECKBOX_SANDBOX_LICENSE_KEY/PIN` в env). **Переписан 2026-09-12 под форму реальной аварии** (до этого слал `go-offline` текущим временем и один чек секундой позже, то есть не проверял ничего из того, ради чего нужен): теперь `go-offline` датируется на N минут назад (по умолчанию 90), затем **три** чека с интервалом в десятки минут, тоже задним числом и сцепленные `previous_receipt_id`, затем `go-online` и перечитывание каждого чека. Отвечает на два вопроса: принимает ли Checkbox `go_offline_date`/`fiscal_date` **в прошлом** (если отказ — скрипт тут же повторяет тот же вызов с `now`, чтобы отличить «дело в дате» от «дело в состоянии кассы») и приходят ли `control_number` и `tax_url` в ответе. Осталось запустить и закоммитить фикстуры `src/__tests__/fixtures/checkbox/offline_*.json` | 1–2 дня | 🟡 ждёт прогона и ответа поддержки |
