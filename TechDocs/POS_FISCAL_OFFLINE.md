...
|---|---|---|---|
| **0. Исследование на песочнице** | Документация снята (`checkbox-api/`); письмо в поддержку Checkbox по вопросам 1–2 ниже отправлено 2026-09-11; прогон на тестовой кассе — скрипт `npm run fiscal:sandbox:offline -- --ask --go [--outage=N] [--sales=N]` (`CHECKBOX_SANDBOX_LICENSE_KEY/PIN` в env). **Переписан 2026-09-12 дважды.** Сначала под форму аварии (было: `go-offline` текущим временем и один чек секундой позже — не проверялось ничего). Затем — под **реальное ожидание**, потому что у каждого офлайн-кода есть собственный `created_at` от налоговой: прогон, который забрал коды в 04:36 и объявил аварию с 03:06, штамповал бы чеки номерами, которых тогда ещё не существовало. В проде так не бывает — касса арендует коды, **пока онлайн**, а авария начинается после, — поэтому такой прогон ничего бы не доказал. Теперь скрипт живёт аварию в реальном времени: берёт коды → ждёт → пробивает `--sales=N` чеков с настоящими отметками времени (ничего не отправляя) → ждёт до конца `--outage=N` минут → и только на «восстановлении связи» шлёт `go-offline`, датированный за секунду до первого чека (к этому моменту он honestly в прошлом), затем сами чеки с их реальными временами, сцепленные `previous_receipt_id`, затем `go-online` и перечитывание. Отвечает на два вопроса: принимает ли Checkbox `go_offline_date`/`fiscal_date` **в прошлом** (при отказе скрипт тут же повторяет вызов с `now` — отличить «дело в дате» от «дело в состоянии кассы») и приходят ли `control_number` и `tax_url`. Печатает также, были ли коды выданы **до** начала сессии — см. открытый вопрос ниже. Осталось запустить и закоммитить фикстуры `src/__tests__/fixtures/checkbox/offline_*.json` | 1–2 дня | 🟡 ждёт прогона и ответа поддержки |
...
Делать это вслепую не стоит: сначала прогон (скрипт печатает, были ли коды
выданы до начала сессии), потом решение. Если Checkbox ограничение не
проверяет — достаточно записи в доке.
