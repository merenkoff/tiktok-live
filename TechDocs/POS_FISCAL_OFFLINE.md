...
1. `goOffline(at = started_at, code = go_offline_code)` — один раз на сессию
   (идемпотентно: сессия помнит, что уже переведена);
...
3. когда очередь сессии пуста и держатель сообщил «снова онлайн» (касса — по
   факту синка; сервер — по факту `preflight` ok) → `goOnline`, сессия
   `closed`, `used`-коды помечены, пул пополнен.
...
2. Обязателен ли `go-offline` перед `sell-offline` при уже открытой онлайн
   смене, и что делает `POST /receipts/sell` с других касс, пока регистратор в
   manual offline.
