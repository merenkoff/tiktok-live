...
|---|---|---|---|
| **0. Исследование на песочнице** | Документация снята (`checkbox-api/`); **прогон на демо-кассе сделан 2026-09-12**, фикстуры `offline_*.json` в репозитории, результаты, два найденных бага и повторный прогон после правок (цепочка из трёх чеков дошла целиком) — в разделе «Прогон на демо-кассе» ниже. Скрипт `npm run fiscal:sandbox:offline -- --ask --go [--outage=N] [--sales=N]` живёт аварию в реальном времени (коды → ожидание → чеки с настоящими отметками → ожидание → реконнект с `go-offline` задним числом) и хранит каждый прогон в `fixtures/checkbox/runs/<метка времени>/`. Осталось (фаза 5): дубль того же кода, `go-online` с недоставленными документами, вторая касса, разрыв смены внутри аварии | 1–2 дня | ✅ 2026-09-12 |
...
### Результат (2026-09-12, PR #75)

Все пять шагов сделаны в одной ветке.

- **Бэкенд.** `FiscalProvider.fetchRequisites` (Checkbox: `/cash-registers/info`
  + `/cash-registers/{id}` + `/tax` → один `FiscalRequisites`, у каждой ставки
  есть `code` — то, что хранит `default_tax_code`); кэш `requisites` /
  `requisites_fetched_at` (миграция 030), TTL 24 ч, `refreshRequisites` —
  никогда не бросает. Точки загрузки: `openShift`, пополнение пула,
  `test-connection` (принудительно). ФН ПРРО зеркалится в
  `register_fiscal_number`. Публичный блок `auth.store.fiscal` несёт
  `register_fiscal_number`, `default_tax_code`, `requisites`.
- **Гейт.** Офлайн-штамп без реквизитов → `unavailable` «Немає звʼязку з
  ПРРО, а реквізити чека ще не отримано — спершу проведіть один чек онлайн».
- **Админка.** `FiscalSettingsCard` → секция «Реквізити ПРРО» (read-only:
  організація, ПН/ІД, точка, адреса, ФН ПРРО, ставки; «Оновити з ПРРО»).
- **Макет** (`receipt.rs` и `ReceiptPrintable.tsx`, один порядок): вывеска
  магазина → рядки 1–5 из реквизитов → позиции с буквой ставки (11) → «СУМА»
  (20), «Знижка», «ПДВ <буква> <ставка>%» (21, считаем из цены с ПДВ:
  `total × rate / (100 + rate)`), «ДО СПЛАТИ» (24) → «ГОТІВКА» /
  «БЕЗГОТІВКОВА (картка|QR)» / «ІНШЕ» (18), «РЕШТА» при переплате готівкою
  (25) → фискальный блок: «ФІСКАЛЬНИЙ ЧЕК» + «ПРРО Checkbox» (35), «ОНЛАЙН»
  / «ОФЛАЙН» (31), «ЧЕК №» (26), «Контрольне число» когда есть (32), «ФН
  ПРРО» (34), дата (27), QR (29) или «QR буде після синхронізації з ПРРО».
  Буква ставки — по `default_tax_code` магазина, иначе `is_default`
  провайдера. Все новые поля `ReceiptData` опциональны / `#[serde(default)]`,
  старая касса печатает как раньше. `fiscalBlockComplete` не менялся.
- Rust проверен в отдельном крейте (`escpos` + `serde`, без `printers`/Tauri):
  9 тестов зелёные; CI Rust не собирает.

Не вошло (следующий заход):
- буква ставки **по позиции** — `SaleDetail.items` не несёт `fiscal_tax_code`
  товара, вся корзина печатается под ставкой магазина; при товарах с другой
  ставкой рядок 21 будет неверен;
- валюта в рядках 18/24 («ГРН»), рядок 23 «заокруглення»;
...
Все пять шагов сделаны. **Три решения плана пересмотрены по ходу кода** —
причины ниже, они важнее самих правок.

1. **`offline_seq` назначает сервер (MAX+1), а реплей шлёт по `fiscal_date`,
   а не по `seq`** (было решение 3: «seq кассы авторитетен»). План этого не
   учёл: пока очередь кассы разгружается, она продолжает продавать **онлайн**,
   и эти чеки тоже штампуются в ту же сессию (иначе доставленный онлайн-чек
   внутри сессии ломает порядок `go-offline`). Значит в одной сессии
   перемешаны документы двух источников, и `seq` кассы с ними столкнётся —
   вторая отключка начинает нумерацию с 1. Порядок цепочки для ДПС задают
   **даты на чеках**, поэтому реплей сортирует по `fiscal_date`, а `seq`
   остался тай-брейкером и диагностикой (`listSessionDocuments`).
2. **Пол для `go-offline` — не только доставленные онлайн-чеки**
   (`offlineChainFloor`): в него вошли `last_go_online_at` прошлой сессии и
   **`closed_at` закрытой смены**. Причина: смена, которую крон автозакрыл,
   пока касса была офлайн, отправляет Z-отчёт позже дат этих чеков —
   без этого сессия ушла бы в ДПС «за Z» молча. Теперь она паркуется `stuck`.
3. **`closeDueShifts` не трогали** (было решение 6: не закрывать смену
   молчащего держателя). «Молчит» неотличимо от «выключена на ночь», а
   не закрывать смену — тоже нарушение. Перекрытие и так пустое: касса
   перестаёт штамповать за 15 мин до дедлайна (`shift_deadline`), сервер
   закрывает **после** него. Остаточный случай (касса офлайн двое суток)
   ловится полом из п. 2 — сессия паркуется, а не портит регистратор.

Что сделано:

- **Бэкенд.** Миграция 031 (`client_session_id`, `ready_at`);
  `offline/lease.ts` + `POST /fiscal/offline/lease` (только держатель,
  идемпотентно, пополнение до `OFFLINE_LEASE_SIZE = 50`, `outbox_pending`
  освобождает реплей); `sessionForDeviceDocument` / `stampDeviceNext` /
  `markDeviceReady` / `openSession` (обобщён из `openServerSession`);
  `POST /sales/complete` принимает `fiscal_offline`; `listReplayableSessions`
  (серверные — всегда, кассовые — по `ready_at` или после часа тишины,
  `DEVICE_READY_GRACE_MS`); новый kind ошибки `offline_code_invalid`;
  `offlineGate` больше не отказывает держателю при живой кассовой сессии.
  Тесты: `pos.fiscal.offline.lease.test.ts` (14), `…offline.device.test.ts` (15).
- **Касса.** `offline/lease.ts` (резерв, гейты, штамп, `leaseSummary`),
  `lib/taxUrl.ts` (копия серверного билдера), `repository.completeSale`
  штампует вместо `throw`, `sync` шлёт `fiscal_offline` и в конце обновляет
  аренду, баннер кассира говорит «Офлайн — чеки ПРРО з резерву (N)» либо
  причину отказа, `OfflinePanel` — «Із них на цій касі: N».
  Тесты: `offline/lease.test.ts` (18), `lib/taxUrl.test.ts` (4),
  `offline/repository.offline-sale.test.ts` (5); `lease.ts` и `taxUrl.ts`
  добавлены в гейт покрытия.
- **Заодно исправлена ошибка фазы 8в:** кэш холодного входа (`StaffUnlockRow`)
  хранил из блока `fiscal` только `enabled`/`provider`, поэтому касса после
  перезапуска не имела реквизитов, ФН ПРРО и кода ставки — и напечатала бы
  офлайн-чек с пустой шапкой. Теперь кэшируется весь публичный блок.

Проверки: бэкенд — typecheck, lint, **1029 тестов** на настоящем Postgres;
POS — typecheck, lint, **607 тестов**, гейт покрытия.

Не вошло (фаза 5): офлайн-возвраты и служебные чеки (§7), вторая касса на
регистратор, e2e-прогон на реальном железе, ручной разбор «непроведених»
чеков в UI владельца (сейчас они видны в списке внимания как документы
сессии), проверка часов кассы (решение 7 — сознательно не полицейская).
...
**Статус после фазы 8в (2026-09-12):** таблица выше — снимок *до* неё.
Теперь рядки 1–5, 11, 18, 20, 21, 24, 25, 26, 31, 34, 35 закрыты нашим
макетом (см. «Результат» фазы 8в); открытыми остаются 32 и `mac` в 29 для
офлайн-чека до реплея (это Checkbox), валюта в 18/24 и буква ставки по
позиции.

Итог на момент сверки был честный и неприятный: **наш локальный макет не
соответствовал Положенню даже для онлайн-чека** — не хватает юридических реквизитов магазина (2, 3,
...
2. **Фаза «8а-відповідність»** — стала фазой 8в, сделана 2026-09-12 (юр.
   реквизиты не вводятся, а берутся у провайдера). Исходная формулировка:
   довести локальный макет до п. 2 — юр. реквизиты магазина (новые поля `pos_stores`: назва та адреса
...
## Прогон на демо-кассе (фаза 0, 2026-09-12) — что ответил Checkbox

Владелец прогнал `npm run fiscal:sandbox:offline -- --ask --go --outage=10
--sales=3` на тестовом регистраторе `TEST938114`. Ответы в фикстурах
`src/__tests__/fixtures/checkbox/offline_*.json`. Прогон подтвердил основу
фазы 3 и **нашёл два бага в нашем коде** — ровно то, ради чего он нужен.

### Подтверждено

1. **`go-offline` задним числом принимается.** Дата на 8 минут в прошлом →
   `{"status":"ok"}`. На этом держится вся фаза 3: касса сообщает об аварии
   только когда та закончилась, поэтому сервер всегда датирует сессию назад.
2. **`fiscal_date` в прошлом принимается и сохраняется как есть.** Чек,
   пробитый в 04:58 и отправленный в 05:06, вернулся с
   `fiscal_date: 2026-09-12T04:58:50.610000+00:00` и `created_at` = время
   отправки. Обе даты Checkbox хранит отдельно.
3. **`control_number` и `tax_url` приходят, когда клиент не слал ничего.**
   `control_number: "7078"` — четыре цифры, как в Положенні № 13 (рядок 32).
   `tax_url` — с `mac`, которого мы посчитать не можем, что и было записано.
4. **Формат `tax_url` у Checkbox:** `?id=…&date=20260912&time=07%3A58%3A50&fn=TEST938114&sm=1.00&mac=…`
   — киевское местное время (чек в 04:58 UTC = 07:58), `HH:MM:SS`. Наш
   `taxUrl.ts` строит `HHmm` и тоже киевское. Оба варианта кабинет принимает
   (реальный чек магазина с `time=1840` проверяется), так что менять не стали;
   расхождение зафиксировано.

### Найдено и исправлено

5. **`previous_receipt_id` слать нельзя.** Чек 1 без указателя принят; чек 2 с
   `id`, который Checkbox только что вернул для чека 1 (и который был
   последним чеком регистратора по `serial`), отвергнут: 400
   `receipt.previous_id_last_id_differs`, «id попереднього чека відрізняється
   від останнього збереженого чека». Что Checkbox считает «останнім
   збереженим чеком» — не то, что мы можем вычислить.

   **Чем это грозило в проде:** реплей слал этот указатель начиная со второго
   документа сессии. Код 400 не входит ни в один наш список, то есть
   классифицировался как `unknown` — не терминальный, — и документ повторялся
   бы каждые две минуты до исчерпания попыток, а потом уходил в список
   внимания. **Все чеки аварии, кроме первого, не дошли бы до ДПС.**

   **Исправление:** поле необязательное, поэтому не шлём его вообще
   (`providers/checkbox/payload.ts`), реплей перестал его вычислять,
   `ledger.lastDoneProviderDocId` удалён. Порядок цепочки задают даты чеков.

6. **Чек не может быть датирован раньше открытия смены.** В фикстуре более
   раннего прогона: 400 `date.fiscal_date_logic`, «Час фіскалізації чека
   повинен бути більше ніж час відкриття зміни (2026-09-12T03:42:34…)».

   **Чем это грозило:** касса, простоявшая офлайн через границу смены, отдаёт
   чеки старше той смены, которая открыта на момент синка. Каждый был бы
   отвергнут поштучно.

   **Исправление:** `offlineChainFloor` теперь включает `opened_at` открытой
   смены. Сессия, которая начинается раньше неё, паркуется `stuck` один раз, с
   причиной, вместо тихой потери чеков по одному.

### Повторный прогон после правок (тот же день, 22:44–22:56Z)

Прогон повторён скриптом без `previous_receipt_id` — и прошёл целиком.
Фикстуры верхнего уровня `offline_*.json` — именно от него.

7. **Цепочка из трёх чеков доставляется.** Авария 10 минут, чеки пробиты в
   22:47:13, 22:49:43 и 22:52:13, отправлены пачкой в 22:54:44 — все три
   приняты (`CREATED`), у каждого свой `control_number` (`0603`, `4356`,
   `3363`) и `tax_url` с `mac`, `fiscal_date` сохранены как пробиты. То есть
   баг 5 действительно был единственной причиной прошлого отказа, а порядок
   цепочки задаётся датами, без указателя на предыдущий чек.
8. **`go-online` вернул кассу в онлайн**, после чего все три чека
   перечитаны. Сколько опросов на это нужно — не обобщаем: в этом прогоне
   хватило первого, но прогон был один, а документация Checkbox прямо
   допускает, что `go-online` придётся повторить (лимит — 1 запрос в 2
   минуты). Наш реплей так и устроен.
9. **Полночь по Киеву в QR.** Чек с `fiscal_date` 22:47:13Z вернулся с
   `tax_url`, где `date=20260913` и `time=01:47:13` — Checkbox переводит в
   киевское время и переходит на следующие сутки. Наш `taxUrl.ts` считает
   так же (`Intl` с `Europe/Kyiv`), отличие прежнее и только в формате
   времени (`HHmm` против `HH:MM:SS`).

**Осталось непроверенным:** повторный `sell-offline` тем же кодом (что
отвечает касса на дубль), поведение `go-online` при неотправленных
документах, вторая касса на одном регистраторе и разрыв смены внутри
аварии. Это задачи фазы 5 (hardening).

### Прогоны не затирают друг друга

Неудачный прогон и удачный шли с разницей в минуты, скрипт писал фикстуры
под одними и теми же именами — файлы первого пропали. Уцелели только
`offline_receipt_sell_offline_{1,2}_error.json`: успех пишет другие имена,
чем отказ. Исправлено — каждый прогон складывает свои ответы, `run.json`
(флаги, код выхода, выводы) и полный `console.log` в
`src/__tests__/fixtures/checkbox/runs/<метка времени>/`, а верхний уровень
остаётся последним прогоном. Заодно скрипт перестал считать HTTP 200 на
`go-offline` доказательством: он перечитывает `info.offline_mode` и говорит,
если касса на самом деле не перешла в офлайн.

## Тексты экрана передачи кассы принадлежат статье (2026-09-14)

Формулировки на `/fiscal` в `HolderPanel` процитированы в публичной статье
`/dovidka/zmina-prro-zamina-kasy` дословно: «Каса зайнята пристроєм A з 09:12»,
«Пристрій B просить передати касу», «Каса не відповідає, можливо продає
офлайн», «Запросити передачу», «Передати касу», «Закрити зміну (Z-звіт) перед
передачею», «Забрати касу примусово».

Это не совпадение и не украшение: статья писалась под поисковый и AEO-разбор,
человек ищет по тому, что видит на экране. Поэтому **правится программа под
статью, а не наоборот**. Если строка на экране меняется — меняется и статья,
тем же коммитом; тесты `HolderPanel.test.tsx` пинуют их дословно.

## Z-звіт при передаче кассы — опция, по умолчанию выключенная (2026-09-13)

Передача замка смену **не** трогает: подмена компьютера не должна резать день
на два Z-звіта (§3а). Но касcир, который и так закрывает день, теперь может
попросить об этом прямо на экране передачи — чекбокс «Закрити зміну (Z-звіт)
перед передачею» рядом с кнопкой, **снят** по умолчанию.

Порядок в роуте `/fiscal/register/handover/confirm` (`close_shift: true`)
важен и проверен тестами:

1. `canConfirmHandover` — те же три условия, что и у обычного подтверждения
   (держатель онлайн, очередь пуста, живой офлайн-сессии нет). Вынесено из
   `confirmHandover` именно ради этого шага: Z-звіт ради передачи, которую
   потом отказали бы, разрезал бы день зря.
2. `closeShift` — пока касса ещё держит регистратор.
3. И только потом замок переезжает; в ответе едут `z_report` / `z_report_text`,
   и панель показывает текст так же, как `ShiftPanel`.

Если Z-звіт не прошёл (нет связи с ПРРО) — 502, **замок остаётся на старой
кассе**: иначе вышло бы худшее из двух, передана касса и не закрыта смена.

## Месячный лимит офлайна: 168 часов на регистратор (2026-09-13)

Вторая цифра из Положення № 13 рядом с 36 часами подряд: **168 годин на
календарний місяць** на один ПРРО. 36 часов — свойство одной сессии, их видно
из строки сессии; 168 — свойство месяца регистратора, то есть сумма всех его
офлайн-проміжків.

- **Считает сервер** — `src/pos/fiscal/offline/limits.ts`. Сумма пересечений
  `[started_at, COALESCE(ended_at, now)]` с календарным месяцем **по Киеву**
  (`kyivMonthStart`, смещение берётся то, что действовало 1-го числа, — в марте
  и октябре оно отличается от сегодняшнего). Считаются все сессии месяца, и
  случай B, и случай C, в любом статусе: `stuck` — это тоже отработанный
  офлайн. `OFFLINE_MONTH_MAX_MS` = 168 ч − 1 ч запаса, как 30 минут запаса у
  36-часового лимита.
- **Сознательный перебор:** `ended_at` проставляется в момент реплея, а не
  когда вернулась сеть. Счётчик слегка завышает — касса встанет раньше закона,
  а не позже.
- **Где запрещает.** `preflight` не открывает новую офлайн-сессию, когда месяц
  исчерпан (`offline_month_limit`, 503). Внутри живой сессии продажа тоже
  отбивается — но, в отличие от 36 часов, сессия **не** паркуется `stuck`: её
  чеки законны и обязаны дойти до ДПС, исчерпано право продавать дальше, а не
  эта цепочка.
- **Касса считать это сама не может** — офлайн другой кассы того же
  регистратора тратит те же часы. Поэтому число едет к ней в ответе аренды
  (`offline_month` в `POST /fiscal/offline/lease`), а она добавляет только тот
  промежуток, который сейчас живёт (`monthUsedMs` в `pos/src/offline/lease.ts`)
  и отказывает `offline_month_limit`. Ответ без этого поля (старый бэкенд) —
  без счёта и без отказа.
- **Видно людям:** на `/fiscal` в `OfflinePanel` («Офлайн цього місяця: N год
  із 167») и на экране владельца в `FiscalSettingsCard`; жёлтым, когда
  остаётся меньше 12 часов.

Тесты — `src/__tests__/pos.fiscal.offline.limits.test.ts` (месяц по Киеву, клип
по границе месяца, оба запрета, поле в аренде и в настройках) и
`pos/src/offline/lease.test.ts` (отказ на кассе, остаток для баннера).

## Порядок кодов в пуле: `serial_id` и `created_at` — свойства ответа, не кода (2026-09-12)

Раньше здесь стоял открытый вопрос: у кода в ответе `get-offline-codes` есть
`created_at`, значит сессия не может начинаться раньше, чем выдан код, которым
её открывают. Формулировка держалась на том, что `created_at` — дата выдачи
номера налоговой, привязанная к самому номеру.

**Это не так**, и видно это по двум фикстурам одного и того же дня. Код
`TEST-VWvfoM` в ответе утреннего прогона (`git show
f65c8b0:src/__tests__/fixtures/checkbox/offline_get_codes.json`) —
`serial_id: 4`, `created_at: 04:56:20`; он же в ответе вечернего —
`serial_id: 1`, `created_at: 22:44:43`. Один и тот же номер, разные числа.
То есть Checkbox отдаёт порядковый номер и отметку времени **позиции в этом
ответе** (список неиспользованных кодов, старые первыми), а не свойства кода.

Что из этого следует:

- **Ограничения «сессия не раньше выдачи кода» — нет**, по крайней мере в том
  виде, в каком оно было записано: сравнивать нечего. Прогон 22:44 открыл
  сессию кодом из ответа, полученного в тот же момент, и датировал её на 10
  минут назад — принято.
- **Хранить `created_at` не нужно**, миграция не нужна — записывать
  per-response значение как свойство кода было бы просто неверно.
- **Сортировка пула исправлена.** `takeFreeCodes` и `listLeasedCodes`
  (`src/pos/fiscal/offline/pool.ts`) сортировали
  `ORDER BY serial_id ASC NULLS LAST, id ASC`; два пополнения дали бы
  пересекающиеся `serial_id`, и порядок выдачи перестал бы быть порядком
  получения. Теперь `ORDER BY id ASC` — порядок вставки, то есть порядок
  ответа провайдера, в котором неиспользованные коды идут старыми вперёд.
  Колонка `serial_id` остаётся (диагностика), комментарий к ней переписан
  миграцией 032.
...
опциональный контроль последовательности, ~~передаём предыдущий `id`~~ **не
передаём никогда** (прогон 2026-09-12, баг 5 выше).
