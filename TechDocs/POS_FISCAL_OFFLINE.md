...
| **3. Касса без связи (случай C)** | **Пересмотрено 2026-09-11: через Checkbox Kasa** (локальный ПРРО на машине кассы) — второй транспорт адаптера, полный чек с контрольным числом и QR офлайн. Сначала снять спецификацию их local API (см. «Открытые вопросы»), затем план исполнения. Прежний вариант — аренда кодов кассе и чек без контрольного числа — отклонён: этих полей касса посчитать не может | 5–6 дней | ⬜ ждёт спецификации local API |
...
- ESC/POS-макет (`pos/src-tauri/src/hardware/receipt.rs`) печатает «ОФЛАЙН»
  и «Контрольне число», а вместо QR, которого ещё нет, — строку «QR буде
  після синхронізації з ПРРО»: печатать ссылку, которая не пройдёт проверку
  в кабинете ДПС, хуже, чем честно сказать;
...
- **Контрольное число и `mac` для QR — не наши.** `control_number` (4 цифры)
  приходит в ответе `sell-offline`, а `mac` в `tax_url` — это
  `transaction.previous_hash`, хеш **цепочки транзакций ПРРО**
  (`checkbox-api/receipts-offline.md`). Цепочку ведёт тот, кто является ПРРО.
  Checkbox Kasa печатает офлайн-чек с обоими реквизитами именно потому, что
  она сама ПРРО на устройстве; через облачный API ПРРО — их сервер, и касса
  без связи эти два поля взять не может.

QR состоит из `id` (фискальный номер), `date`, `time`, `fn` (ФН ПРРО), `sm`
(сумма) и `mac`. Первые пять касса знает сама; проверяемым QR делает
шестой.

**Решение (2026-09-11, владелец): случай C идёт через Checkbox Kasa** —
локальный компонент на машине кассы (`wiki.checkbox.ua/uk/api/local_api_specification`),
который и есть ПРРО офлайн, поэтому даёт полный чек. Для нас это второй
**транспорт** того же адаптера (локальный base URL вместо облачного; в §
контракта провайдера это уже предусмотрено), а не второй провайдер.

Что нужно до старта фазы 3 — снять спецификацию их local API так же, как
сняты остальные страницы в `checkbox-api/` (из песочницы разработки
`wiki.checkbox.ua` закрыт сетевой политикой). Проверить по ней три вещи:
1. есть ли офлайн-продажа локально и возвращает ли она `control_number` и
   `tax_url`/`mac` без связи с их облаком;
2. кто держит запас кодов — локальный компонент сам или мы его кормим;
3. как устроены смена и Z-отчёт, когда часть чеков прошла через локальный
   компонент, а часть — через облако.
