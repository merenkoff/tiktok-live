...
| **3. Касса без связи (случай C)** | **Через Checkbox Kasa** (локальный ПРРО на машине кассы, их local API) — решено 2026-09-12 по тексту Положення № 13: офлайн-чек обязан нести контрольне число и `mac` в QR, а их формирует только тот, кто ведёт офлайн-сессию ДПС. Облачный путь для C невозможен. Перед этим — фаза «8а-відповідність» (макет чека по п. 2) и снимок спецификации Kasa | 5–6 дней | ⬜ ждёт спецификации Kasa |
...
3. **Фаза 3 (случай C):** полный офлайн-чек в момент продажи = локальный
   ПРРО на кассе. Путь — Checkbox Kasa (их local API), спецификацию снять.
   Облачный путь остаётся для случая B как есть: документ регистрируется
   верно, а покупателю после реплея доступен полный е-чек — это и надо
   дать (кнопка «надіслати е-чек» / допечатка с контрольным числом и QR
   провайдера).
