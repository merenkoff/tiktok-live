...
- ~~Інвентаризація на касі (продавець)~~ — **зроблено 2026-09-09** як online-only модуль `stocktake` зі своїми офлайн-даними (roadmap #12 трек 3): продавець рахує на касі сканером, лист їде чернеткою документа `inventory`, проводить власник на вебі → [[POS_MODULE_OFFLINE_DATA]]
