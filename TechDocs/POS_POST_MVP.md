...
Не в цій поставці (окремі пункти нижче / пізніше): офлайн, ESC/POS, автооновлення, кіоск.

---

## P3. Офлайн каса

**Статус:** не почато. Технологія оболонки вже обрана (Tauri 2); офлайн робити в JS, див. [[POS_DESKTOP]].

- Кэш каталогу + черга продажів (IndexedDB)
- Синк `completeSale` коли з’явиться мережа
- Без переїзду на Electron і без Rust-SQLite на старті
