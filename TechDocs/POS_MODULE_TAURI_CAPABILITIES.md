...
| **Наши команды** | Все 10 из `generate_handler!` ([`src-tauri/src/lib.rs`](../pos/src-tauri/src/lib.rs)): `list_hardware` (перечисление HID-устройств), `list_printers`, `print_receipt` (ESC/POS-байты в любой принтер ОС), `print_kitchen_ticket` (с 2026-09-21, кафе К3e: тот же raw-job в принтер по имени, только текст тикета — без путей и сети, чек-лист §7 пройден как у `print_receipt`), `print_precheck` (с 2026-09-21, столы К4h: предчек — тот же raw-job в принтер по имени, только текст документа; макет намеренно без «ЧЕК №», фискального блока и QR, чек-лист §7 пройден как у `print_receipt`), `print_webview`, `check_for_update`, `install_update` (скачать и поставить обновление + рестарт), `sync_module_remote` (с 2026-09-09 принимает `cachedOnly` — ответ из кеша без сети, и `hostPlatform` — `PLATFORM_VERSION` хоста, манифест с большим `minHostPlatform` не скачивается, см. [POS_MODULE_PLATFORM_VERSION.md](POS_MODULE_PLATFORM_VERSION.md)), `prune_module_remotes(keep)` (удалить кеш модулей не из списка — худшее, что даёт модулю: снести чужой кеш, который перекачается следующим синком). |
