# Опис API Фіскального Сервера ДПС (ПРРО) — снимок

Источник: документ «Опис АРІ Фіскального Сервера (Єдине вікно подання
електронної звітності)» из Электронного кабинета ДПС, снят владельцем
2026-09-12 (docx) и переведён в markdown автоматически (python-docx: заголовки,
абзацы, списки, таблицы). Форматирование XML-примеров могло пострадать —
при сомнении сверяться с исходным docx.

Зачем он в репозитории — см. [README.md](README.md): четыре факта отсюда
определяют, что мы можем и чего не можем сделать с офлайн-чеком без
собственного ПРРО.
...
Якщо покупець відмовляється від проведеної операції (останньої проведеної на ПРРО; для торгівлі валютними цінностями – не обов’язково останньої) і при цьому здійснено реєстрацію чека Фіскальним Сервером, або якщо чек з помилковою сумою зареєстрований Фіскальним Сервером:
...
- Користувач заходить в ПРРО з аутентифікацією по КЕП. ПРРО одержує з Сервера довідники (перелік господарських одиниць (ГО), перелік фіскальних номерів ПРРО, тощо).

- Користувач вибирає доступну ГО і фіскальний номер ПРРО, і дає команду на відкриття зміни. ПРРО надсилає на Сервер документ «Відкриття зміни», і одержує у відповідь фіскальний номер документа, ідентифікатор офлайн сесії та «секретне число».

- Користувач видає чеки.

### ПРРО працює з відкритою зміною (Сервер недоступний)

- Користувач заходить в ПРРО з аутентифікацією по КЕП.

- Користувач заходить в ГО і фіскальний номер ПРРО, для якого відкрито офлайн сесію.

- ПРРО перевіряє, чи зміну відкрито тим самим користувачем. Якщо ні – робота Користувача неможлива до закриття зміни.
...
- Користувач заходить в доступну ГО і фіскальний номер ПРРО, для якого відкрито офлайн сесію, і дає команду на відкриття зміни.

- Користувач видає чеки.

- Користувач закриває зміну.

### Перевірка зв’язку з Сервером (ПРРО знаходиться в режимі онлайн)

- Якщо при передачі документа Сервер недоступний, ПРРО запитує у користувача підтвердження переходу в режим офлайн.

- Якщо користувач згодний, ПРРО створює документ «Початок офлайн сесії»

### Перевірка зв’язку з Сервером (ПРРО знаходиться в режимі офлайн)

- ПРРО періодично перевіряє зв’язок з Сервером (наприклад, за допомогою команди «Запит стану ПРРО», або команди «Запит стану сервера»).

- Якщо зв’язок присутній, ПРРО автоматично переходить в режим онлайн, про що інформує користувача.

- ПРРО надсилає на Сервер пакет, що містить документи, створені в режимі офлайн.

- У разі відмови Сервера на підставі некоректного сертифікату, пакет засвідчується підписом старшого користувача або керівника.

- Якщо пакет успішно прийнятий, ПРРО отримує підтвердження (квитанцію) від фіскального Сервера.

### Скасування реєстрації ПРРО

- Користувач надсилає заяву з позначкою «Скасування реєстрації»

- Користувач надсилає повідомлення з позначкою «несправність» або «крадіжка пристрою»

- Автоматичне скасування (припинено державну реєстрацію, закрито господарську одиницю тощо).
...
“OfflineSessionRollbackCmdUID”: <Унікальний ідентифікатор запиту, за яким було здійснено відміну закриття офлайн сесії (null – режим офлайн заборонений для ПРРО)>,
...
У запиті зазначається або фіскальний, або локальний номер документа.
...
“Type”: <Тип даних запиту документа>,
...
“ShiftId”: <Ідентифікатор зміни, до якої належить документ>,

“ResultCode”: “<Код результату обробки запиту документа>”,

“ResultText”: “<Опис результату обробки запиту документа>”,
...
“NumLocal”: <Локальний номер Z-звіту>,
...
Якщо зазначений ідентифікатор зміни “ShiftId”, то поля “From” і “To” не використовуються.
...
“Document”: “<Документ в кодуванні Base64>”,

“UID”: “<Унікальний ідентифікатор запиту>”

}

Елемент “Document” містить документ, створений згідно з розділом “Загальний порядок взаємодії ПРРО з Фіскальним Сервером”.
...
“Ticket”: “<Квитанція в кодуванні Base64>”
...
“Command”: “SendOflPackage”,

“Documents”: <Масив офлайн документів в кодуванні Base64>,

“UID”: “<Унікальний ідентифікатор запиту>”

}

Масив “Documents” містить документи пакета в послідовності локальних номерів. Див. розділ “Пакет офлайн документів”.

Кожен елемент масиву містить окремий документ.
...
“OfflineSessionId”: <Ідентифікатор офлайн сесії (надсилається у відповідь на пакет, який містить документ «Завершення офлайн сесії»)>,

“OfflineSeed”: <"Секретне число" для обчислення фіскального номера офлайн документа (надсилається у відповідь на пакет, який містить документ «Завершення офлайн сесії»)>
...
- ПРРО визначає унікальний ідентифікатор пристрою.

- Унікальний ідентифікатор пристрою представлений 64-символьним значенням. Наприклад, SHA-256 від MAC адреси мережевої карти.
...
Повідомлення повинне бути засвідчене кваліфікованим електронним підписом відповідно до вимог Закону України «Про електронні довірчі послуги» (див. https://zakon.rada.gov.ua/laws/show/2155-19).
