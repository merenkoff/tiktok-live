...
export default defineConfig({
  ...
  webServer: {
    ...
    // `build:stocktake-remote` / `build:vertical-flowers-remote`: those specs
    // serve the real, signed bundles from a fake CDN and drive them inside the
    // web shell.
    command:
      'VITE_REMOTE_ALLOW_DEV_KEY=1 npm run build && npm run build:stocktake-remote && npm run build:vertical-flowers-remote && npm run preview -- --host localhost --port 4173',
